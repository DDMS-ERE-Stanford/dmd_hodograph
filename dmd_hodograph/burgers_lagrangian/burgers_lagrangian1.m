...
A = (1+4*dt/dx)*diag(ones(Nx,1))-2*dt/dx*diag(ones(Nx-1,1),-1)-2*dt/dx*diag(ones(Nx-1,1),1);
A(1,:) = 0; A(1,1) = 1;
A(end,:) = 0; A(end,end) =1;


for i = 2:Nt
%     Y2(:,i) = A\Y2(:,i-1); % artificial diffuison
    Y2(:,i) = Y2(:,i-1);
%     Y2(:,i) = Y2(:,i-1)+dt*2/dx*(circshift(Y2(:,i-1),-1)-2*Y2(:,i-1)+circshift(Y2(:,i-1),1));
%     need small dt
    Y1(:,i) = Y1(:,i-1)+dt*(Y2(:,i-1)+Y2(:,i))/2;
end
X = [Y1;Y2];
